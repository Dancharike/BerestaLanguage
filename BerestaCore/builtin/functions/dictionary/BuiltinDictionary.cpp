...
static bool ensure_dict_arg(const std::vector<Value>& args, size_t idx, const std::string& name)
{
    if(idx >= args.size()) {diag_error(name + ": missing argument #" + std::to_string(idx)); return false;}
    if(args[idx].type != ValueType::DICTIONARY) {diag_error(name + ": argument #" + std::to_string(idx) + " must be dictionary"); return false;}
    return true;
}

Value BuiltinDictionaryKeys::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 1, 1);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
...
Value BuiltinDictionaryValues::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 1, 1);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    const auto& dict = *std::get<DictionaryPtr>(args[0].data);
...
Value BuiltinDictionaryHas::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 2, 2);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    if(args[1].type != ValueType::STRING) {diag_error("dictionary_has: key must be string"); return {};}
    const auto& dict = *std::get<DictionaryPtr>(args[0].data);
    const auto& key = std::get<std::string>(args[1].data);
    return Value(dict.find(key) != dict.end());
}

Value BuiltinDictionaryGet::invoke(const std::vector<Value>& args)
{
    ensure_min_arity(args, 2);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    if(args[1].type != ValueType::STRING) {diag_error("dictionary_get: key must be string"); return {};}
    const auto& dict = *std::get<DictionaryPtr>(args[0].data);
    const auto& key = std::get<std::string>(args[1].data);
...
Value BuiltinDictionarySet::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 3, 3);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    if(args[1].type != ValueType::STRING) {diag_error("dictionary_set: key must be string"); return {};}
...
Value BuiltinDictionaryDelete::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 2, 2);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    if(args[1].type != ValueType::STRING) {diag_error("dictionary_delete: key must be string"); return {};}
    auto& dict = *std::get<DictionaryPtr>(args[0].data);
    const auto& key = std::get<std::string>(args[1].data);
    dict.erase(key);
    return args[0];
...
Value BuiltinDictionarySize::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 1, 1);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    const auto& dict = *std::get<DictionaryPtr>(args[0].data);
...
Value BuiltinDictionaryClear::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 1, 1);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    auto& dict = *std::get<DictionaryPtr>(args[0].data);
...
Value BuiltinDictionaryCopy::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 1, 1);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    const auto& dict = *std::get<DictionaryPtr>(args[0].data);
...
Value BuiltinDictionaryMerge::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 2, 2);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
    if(!ensure_dict_arg(args, 1, name())) {return {};}
...
Value BuiltinDictionaryDestroy::invoke(const std::vector<Value>& args)
{
    ensure_arity(args, 1, 1);
    if(!ensure_dict_arg(args, 0, name())) {return {};}
